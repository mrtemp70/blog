...
Open **Package Manager Console** and execute the following command. (Old Process: Create a folder (e.g., `_EF_`) → Right-click on the folder → Select `_edmx_`)

```powershell
Scaffold-DbContext "Server=.\SQLEXPRESS;Database=TempoDB;Trusted_Connection=True;Encrypt=True;TrustServerCertificate=True; User Id = sa; Password=123456;" Microsoft.EntityFrameworkCore.SqlServer -OutputDir Models
```

This will generate all entities into the `Models` folder. Now, Open the auto-generated context class and **replace** the `OnConfiguring` method (Safe for Production)
...
{
  "ConnectionStrings": {
    "dbconn": "Server=DESKTOP-45AF10H\\SQLEXPRESS;Database=EOS_DB_First;Trusted_Connection=True;Encrypt=True;TrustServerCertificate=True;"
  }
}
```

Register DB Context. Modify `Program.cs`

```csharp
builder.Services.AddDbContext<EosDbFirstContext>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("dbconn")));
```


Navigate to your **controller** folder and select the controller to auto-generate CRUD operations for the scaffolded models.

![[controller-select-for -dbfirst.png]]
