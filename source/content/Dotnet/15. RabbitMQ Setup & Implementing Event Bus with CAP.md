...
- After starting both projects at the same time, two CAP tables will be automatically generated: `cap.Published` and `cap.Received`. If two database different then it will create two different `cap.Published` and `cap.Received` on each database. To view dashboard on each project `http://localhost:xxxx/cap-dashboard`
